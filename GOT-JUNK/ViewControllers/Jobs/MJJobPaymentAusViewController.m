//
//  MJJobPaymentAusViewController.m
//  GOT-JUNK
//
...
@interface MJJobPaymentAusViewController ()
...
@implementation MJJobPaymentAusViewController
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if ([self.currentJob.junkCharge integerValue] > 0)
    {
        ...
        self.totalField.text = [NSString stringWithFormat:@"%0.2f", [self.currentJob.total floatValue] / 100 ];
...
-(void)savePayments
{
    ...
    float newJCNumber = [totalNumber floatValue];
    newJCNumber = newJCNumber / (1.0 + (10.0/100.0));
    int subTotalNumber = roundf(newJCNumber * 100);
    
    /// TaxID: 721 = 0%, 781 = 10%
    
    NSString *path = [NSString stringWithFormat:@"v1/Job/%@/Payments?sessionID=%@&routeID=%@&dayID=%@&taxID=%d&invoiceNumber=%@&discount=%d&junkCharge=%d&subTotal=%d&total=%d", self.currentJob.jobID, sessionID, routeID, dateString, 781, self.invoiceField.text,dNumber, subTotalNumber+dNumber,subTotalNumber,[totalNumber intValue] * 100 ];
    ...
    self.currentJob.total = [NSString stringWithFormat:@"%d",subTotalNumber];
}
...
- (void)postPaymentSuccessful
{
    ...
    UIAlertView *av = [[UIAlertView alloc] initWithTitle:@"Update Payment Info Successful" message:@"The payment information has been saved into Junknet" delegate:self cancelButtonTitle:@"Close" otherButtonTitles: nil];
    [av show];
}
...
- (void)postPaymentFailed
{
    ...
    UIAlertView *av = [[UIAlertView alloc] initWithTitle:@"Cannot save payment" message:@"Unable to save the payment to Junknet" delegate:self cancelButtonTitle:@"Close" otherButtonTitles: nil];
    [av show];
}
